...
int main(void){
    ...
    t_list_adj* graphe = readGraph("exemple_meteo.txt");
    if(graphe == NULL){
        fprintf(stderr, "Erreur: impossible de lire exemple_meteo.txt\n");
        return 1;
    }
    ...
    //     run_scenario(title, Pi0, P);
    // }
        // 7.1: depart dans l'etat 3 (index 2)
    zero_pi(Pi0);
    Pi0[2] = 1.0f;
    run_scenario("Q7.1: depart dans l'etat 3 (e3)", Pi0, P);

    // 7.2: depart uniforme sur {3,7,23} (indices 2,6,22)
    zero_pi(Pi0);
    Pi0[2] = 1.0f/3.0f;
    Pi0[6] = 1.0f/3.0f;
    Pi0[22]= 1.0f/3.0f;
    run_scenario("Q7.2: depart uniforme sur {3,7,23}", Pi0, P);

    // 7.3: trois tirages aleatoires sur {3,7,23}
    for(int trial = 1; trial <= 3; trial++){
        float a = (float)rand() / (float)RAND_MAX;
        float b = (float)rand() / (float)RAND_MAX;
        float c = (float)rand() / (float)RAND_MAX;
        ...
        Pi0[22] = c / s;
        char title[80];
        snprintf(title, sizeof(title), "Q7.3: aleatoire #%d sur {3,7,23} (a=%.3f,b=%.3f,c=%.3f)", trial, Pi0[2], Pi0[6], Pi0[22]);
        run_scenario(title, Pi0, P);
    }
...
// #define N_STATES 5   // météo = 5 états
// #define MAX_ITER 1000

// /* Multiplie un vecteur ligne par une matrice M */
// void mult_vect_matrix(float* res, const float* vect, t_matrix M){
//     for(int j = 0; j < M.nb_colonnes; j++){
//         float s = 0.0f;
//         for(int i = 0; i < M.nb_lignes; i++){
...
// void run_scenario(const char* titre, const float* Pi0, t_matrix P){
//     int tests[] = {1,2,10,50};
//     printf("\n=== %s ===\n", titre);
...
// int main(void){
//     /* Lecture du graphe météo : 5 états */
//     t_list_adj* graphe = readGraph("exemple_meteo.txt");
//     if(graphe == NULL){
//         printf("Erreur de lecture fichier exemple_meteo.txt\n");
//         return 1;
//     }
//     t_matrix P = create_matrix(graphe);
...
//     /* ----- SCÉNARIO 1 : aujourd’hui SOLEIL (état 1) ----- */
//     zero_pi(Pi0);
//     Pi0[0] = 1.0;
//     run_scenario("Meteo - Depart Soleil", Pi0, P);

//     /* ----- SCÉNARIO 2 : aujourd’hui PLUIE (état 3) ------ */
//     zero_pi(Pi0);
//     Pi0[2] = 1.0;
//     run_scenario("Meteo - Depart Pluie", Pi0, P);

//     /* ----- SCÉNARIO 3 : distribution uniforme sur les 5 états ----- */
//     zero_pi(Pi0);
//     for(int i = 0; i < N_STATES; i++) Pi0[i] = 1.0 / N_STATES;
//     run_scenario("Meteo - Uniforme sur {S,N,P,O,E}", Pi0, P);
...
// /* Affiche seulement les composantes de C1 et quelques sommations utiles */
// void print_C1_state(const float* Pi, const int* C1_idx, int C1_n){
//     /* Affiche Pi pour les états de C1 */
//     printf("Pi(C1) = ");
//     for(int k = 0; k < C1_n; k++){
//         int idx = C1_idx[k];
//         printf("%.6f ", Pi[idx]);
//     }
//     /* Somme totale sur C1 */
//     float sumC1 = 0.0f;
//     for(int k = 0; k < C1_n; k++) sumC1 += Pi[C1_idx[k]];
//     /* Sous-classes : S0 = {1,18,26} -> indices 0,17,25 ; S1 = {4,11,27} -> indices 3,10,26 */
//     float sumS0 = Pi[0] + Pi[17] + Pi[25];
//     float sumS1 = Pi[3] + Pi[10] + Pi[26];
//     printf("| sumC1=%.6f S0=%.6f S1=%.6f\n", sumC1, sumS0, sumS1);
// }

// /* Itère la chaîne complète et affiche les étapes choisies mais seulement pour C1 */
// void run_C1_check(const char* title, const float* Pi0, t_matrix P){
//     printf("\n=== %s ===\n", title);

//     /* indices 0-based des états C1 = {1,4,11,18,26,27} */
//     int C1_idx[6] = {0,3,10,17,25,26};
//     int C1_n = 6;

//     /* étapes à afficher (ajoute ou retire si tu veux) */
//     int steps[] = {1,2,10,20,21,30,31,40,41,50,51};
//     int nsteps = sizeof(steps)/sizeof(steps[0]);
//     int next_print = 0;
//     int max_step = steps[nsteps-1];

//     float Pi[N_STATES], temp[N_STATES];
//     for(int i=0;i<N_STATES;i++) Pi[i] = Pi0[i];

//     for(int n=1; n<= max_step; n++){
//         mult_vect_matrix(temp, Pi, P);
//         for(int i=0;i<N_STATES;i++) Pi[i] = temp[i];

//         if(next_print < nsteps && n == steps[next_print]){
//             printf("Pi(%d): ", n);
//             print_C1_state(Pi, C1_idx, C1_n);
//             next_print++;
//         }
//     }
// }
...
// int main(void){
//     /* lire la matrice 27x27 depuis ton fichier */
//     t_list_adj* graphe = readGraph("matrice27projet.txt"); // <--- ton fichier
//     if(graphe == NULL){
...
//     /* vérification rapide : chaque ligne doit sommer ~1 */
//     for(int i=0;i<N_STATES;i++){
//         float s = 0.0f;
//         for(int j=0;j<N_STATES;j++) s += P.donnees[i][j];
//         if (fabsf(s - 1.0f) > 1e-5f){
//             fprintf(stderr, "Warning: ligne %d somme = %f (!=1). Verifie la matrice.\n", i+1, s);
//         }
//     }

//     float Pi0[N_STATES];
//     /* --- CAS 1 : depart dans l'etat 1 (indice 0) --- */
//     zero_pi(Pi0);
//     Pi0[0] = 1.0f;
//     run_C1_check("Depart etat 1 (e1)", Pi0, P);

//     /* --- CAS 2 : depart uniforme sur C1 --- */
//     zero_pi(Pi0);
//     Pi0[0] = 1.0f/6.0f;  // état 1
//     Pi0[3] = 1.0f/6.0f;  // état 4
//     Pi0[10]= 1.0f/6.0f;  // état 11
//     Pi0[17]= 1.0f/6.0f;  // état 18
//     Pi0[25]= 1.0f/6.0f;  // état 26
//     Pi0[26]= 1.0f/6.0f;  // état 27
//     run_C1_check("Depart uniforme sur C1", Pi0, P);

//     /* --- CAS 3 : depart non uniforme --- */
//     zero_pi(Pi0);
//     /* 0.7 sur S0 = {1,18,26} ; 0.3 sur S1 = {4,11,27} */
//     Pi0[0]  = 0.7f/3.0f;  // 1
//     Pi0[17] = 0.7f/3.0f;  // 18
//     Pi0[25] = 0.7f/3.0f;  // 26
//     Pi0[3]  = 0.3f/3.0f;  // 4
//     Pi0[10] = 0.3f/3.0f;  // 11
//     Pi0[26] = 0.3f/3.0f;  // 27
//     run_C1_check("Depart non uniforme (0.7 S0 / 0.3 S1)", Pi0, P);
