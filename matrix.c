...
t_matrix subMatrix(t_matrix matrix, t_list_class* partition, int compo_index) {
    // saisie sécurisée
    if (compo_index < 0 || compo_index >= partition->taille) {
        printf("Erreur: indice de composante invalide %d (max: %d)\n", compo_index, partition->taille - 1);
        return create_empty_matrix(0);
    }

    // on récupère la classe demandée
    t_class* classe = partition->tab[compo_index];
    int size = classe->size;

    // Création la sous-matrice
    t_matrix sub = create_empty_matrix(size);

    // on collecte les indices des sommets dans la classe
    int* indices = (int*)malloc(size * sizeof(int));
    for (int i = 0; i < size; i++) {
        indices[i] = classe->nb_vertex[i].id - 1; // -1 pour index 0-based
    }

    // ici on remplit la sous matrice avec les valeurs données
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            sub.donnees[i][j] = matrix.donnees[indices[i]][indices[j]];
        }
    }
